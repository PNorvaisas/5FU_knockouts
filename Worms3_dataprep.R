...
#Get Gene names by Plate-Well positions
...
bact<-merge(bac,subset(keioinfo,!Gene %in% c('dcuC','yedN'))[,c('Gene','Plate','Well')],by='Gene',all.x=TRUE)
...
#intersect(unique(bac$Gene),rdupl)

#intersect(unique(bac3a$Gene),rdupl)
...
bacs$OD<-bacs$OD_raw-0.057392708
bacavg<-ddply(bacs, .(Plate,Well,Gene,Drug), summarise, NGM=mean(OD,na.rm=TRUE),NGM_sd=sd(OD,na.rm=TRUE)) #Plate,Well,Drug,
bacavg<-subset(bacavg,! Gene %in% c('XXXXXXX','no bacteria','empty'))

#bothscr<-subset(bacavg,Gene %in% c('upp','hyfF','WT','hyfB'))
#bothscrall<-merge(subset(bothscr,Drug==0),subset(bothscr,Drug==100),by=c('Plate','Well','Gene','Screen'),suffixes = c("_C","_D"))
#bothscrall$Drug_C<-NULL
#bothscrall$Drug_D<-NULL
#write.csv(bothscrall,paste(ddir,'/2nd-3rd_comparison.csv',sep=''))


scrcomp<-ggplot(bothscrall,aes(x=NGM_C,y=NGM_D,color=Screen))+
  geom_point(size=1)+ylim(0, .25)+xlim(0,.25)+
  ...
  labs(color='Screen')
scrcomp
dev.copy2pdf(device=cairo_pdf,file=paste(odir,"/Control-Treatment_NGM_growth_KO-comparison.pdf",sep = ''),width=9,height=9)

  


bacall<-merge(subset(bacavg,Drug==0),subset(bacavg,Drug==100),by=c('Plate','Well','Gene'),suffixes = c("_C","_D"))
bacall$Drug_C<-NULL
bacall$Drug_D<-NULL
...
bacmic<-subset(bacmic,!Gene=='XXXXX')
